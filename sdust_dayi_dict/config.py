...
def config_init(dir):
    ...
    if not os.path.exists(config_path):
        config['WordManagement'] = {
            "default_admins": "851845341,291875783",  # 替换为实际的默认管理员QQ号
            "super_admin": "851845341",  # 替换为实际的超级管理员QQ号
            "database_url": f"sqlite:///{os.path.join(dir, 'data', 'word_management.db')}"
        }
...
def get_database_url(config):
    return config.get("WordManagement", "database_url", fallback="")
